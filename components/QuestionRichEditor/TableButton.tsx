...
export default function TableButton ({ editor }:EditorProps) {
  ...
  const tableControls = [
    { icon: <TbTableColumn className={'w-4 h-4 lg:w-6 lg:h-6'}/>, action: () => editor?.chain().focus().addColumnAfter().run(), title: 'Añadir columna después' },
    { icon: <TbFreezeRowColumn className={'w-4 h-4 lg:w-6 lg:h-6'}/>, action: () => editor?.chain().focus().addRowAfter().run(), title: 'Añadir fila después' },
    { icon: <Trash2 className={'w-4 h-4 lg:w-6 lg:h-6'}/>, action: () => editor?.chain().focus().deleteTable().run(), title: 'Eliminar tabla' },
    { icon: <Merge className={'w-4 h-4 lg:w-6 lg:h-6'}/>, action: () => editor?.chain().focus().mergeCells().run(), title: 'Combinar celdas' },
    { icon: <Split className={'w-4 h-4 lg:w-6 lg:h-6'} />, action: () => editor?.chain().focus().splitCell().run(), title: 'Dividir celda' },
    { icon: <LayoutGrid className={'w-4 h-4 lg:w-6 lg:h-6'}/>, action: () => editor?.chain().focus().toggleHeaderRow().run(), title: 'Alternar fila de encabezado' },
  ]
    ...
    <div className="relative" ref={menuRef}>
      ...
      {isTableMenuOpen  && (
        <div className="absolute z-10 mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg p-4 w-40 lg:w-48 lg:left-0 lg:-translate-x-0 left-1/2 transform -translate-x-1/2">
          ...
          <button
            onClick={insertTable}
            className="w-full text-xs lg:text-sm bg-button-color hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded"
            type={'button'}
          >
            Insertar Tabla
          </button>
